...
class CUTFPNModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--ratios', nargs='+', type=float, default=None, help='scale ratio for extend NCE')
        parser.add_argument('--vflip', type=float, default=0.0, help='possibility to vflip')
        parser.add_argument('--hflip', type=float, default=0.0, help='possibility to hflip')
    ...
    def compute_G_loss(self):
        ...
        if self.opt.lambda_NCE > 0.0:
            ...
            self.loss_NCE, self.loss_NCE_bd, self.loss_extend_NCE = 0.0, 0.0, 0.0

        flip_fake = self.flip_fake
        if self.vertical_flip or self.horizontal_flip:
            pred_flip_fake = self.netD(flip_fake)
            self.loss_G_vGAN = self.criterionGAN(pred_flip_fake, True).mean() * self.opt.lambda_GAN
            self.loss_fake_consistent = self.criterionCycle(self.flip_fake, self.fake_B)
            if self.opt.lambda_NCE > 0.0:
                self.loss_vNCE = self.calculate_NCE_loss(self.real_A, self.flip_fake)
        else:
            self.loss_G_vGAN = 0.0
            self.loss_vNCE = 0.0
        ...
        if getattr(self, 'loss_extend_NCE', None) is not None:
            loss_NCE_both += self.loss_extend_NCE#((self.loss_NCE + self.loss_extend_NCE )+ self.loss_NCE_Y )
        
        if (self.vertical_flip or self.horizontal_flip) and self.opt.lambda_NCE > 0.0:
                loss_NCE_both += self.loss_vNCE

        loss_NCE_both += self.loss_NCE
        # loss_NCE_both /= len(self.extend_fakes) + (3 if self.vertical_flip else 2)
        # else:
        #     loss_NCE_both = self.loss_NCE

        self.loss_G = self.loss_G_GAN + loss_NCE_both
        if self.vertical_flip or self.horizontal_flip:
            self.loss_G +=  self.loss_G_vGAN + self.loss_fake_consistent
        return self.loss_G
